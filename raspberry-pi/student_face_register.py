...
match_post_url = "https://myc-sistech.com/sistem-absensi-uph-web-and-api/api/students.php"
...
def register_face(student_id):
	...
	if response is None:
    	messagebox.showerror("Error",  "Tidak ada respon dari server")
	...
	if encoding:
    	...
    	messagebox.showerror("Error", "Wajah mahasiswa tidak terdeteksi!")
    	return None
...
def open_camera_window(student_id):
	...
	canvas = tk.Canvas(camera_window, width=480, height=300)
	canvas.pack()
...
if __name__ == "__main__":
	if len(sys.argv) > 1:
    	...
    	print("listBoxSelectId:", studentId)
    	open_camera_window(studentId)
	else:
    	print("Error: listBoxSelectId not provided.")
